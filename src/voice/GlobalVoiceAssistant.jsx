...
import { Check, Copy, Link2, ShieldCheck, Sparkles, X } from 'lucide-react';
...
export default function GlobalVoiceAssistant() {
  ...
  const processTranscript = async (transcript) => {
    ...
    const answer = connectedResponse?.answer || result.response;
  ...
  useEffect(() => {
    const toggleVoice = () => activateRef.current?.();
    window.addEventListener('byizon:toggle-voice', toggleVoice);
    return () => window.removeEventListener('byizon:toggle-voice', toggleVoice);
  }, []);

  useEffect(() => {
    window.dispatchEvent(new CustomEvent('byizon:voice-status', {
      detail: {
        status,
        message,
  ...
  }, [lastResponse, lastTranscript, message, status]);

  return (
    <>
      <span className="voice-controller-status" aria-live="polite" role="status">{message}</span>
      {shareResult && (
