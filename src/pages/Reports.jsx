...
export default function Reports() {
  ...
  if (!uploadedData) {
    return (
      <div className="app-layout">
        <Sidebar />
        <main className="main-content reports-page reports-empty-page">
  ...
  const reportOverviewBullets = [
    `${uploadedData.fileName} contains ${Number(uploadedData.rowCount || 0).toLocaleString('en-IN')} records across ${Number(uploadedData.columns?.length || 0).toLocaleString('en-IN')} columns and ${uploadedData.tables?.length || 1} table(s).`,
    `The dataset is classified as ${uploadedData.datasetType || 'a structured business dataset'}${uploadedData.businessDomain ? ` in the ${uploadedData.businessDomain} domain` : ''}.`,
    `Data quality is ${qualitySummary.qualityScore}/100 with ${qualitySummary.completenessScore}% completeness and ${Number(qualitySummary.duplicateCount || 0).toLocaleString('en-IN')} duplicate rows.`,
    `Missing cells: ${Number(qualitySummary.missingCellCount || 0).toLocaleString('en-IN')}; invalid values: ${Number(qualitySummary.invalidValueCount || 0).toLocaleString('en-IN')}; detected outliers: ${Number(qualitySummary.outlierCount || 0).toLocaleString('en-IN')}.`,
    uploadedData.mainEntity ? `The primary business entity is ${uploadedData.mainEntity}.` : null,
    (uploadedData.warnings || uploadedData.dataWarnings || [])[0] ? `Primary warning: ${typeof (uploadedData.warnings || uploadedData.dataWarnings)[0] === 'object' ? (uploadedData.warnings || uploadedData.dataWarnings)[0].message || (uploadedData.warnings || uploadedData.dataWarnings)[0].text : (uploadedData.warnings || uploadedData.dataWarnings)[0]}.` : 'No critical structural warning was detected in the uploaded data.',
  ].filter(Boolean);
    ...
    <div className="app-layout">
      ...
      <main className="main-content reports-page">
        ...
        <div className="report-hero animate-fadeInUp">
          <div className="report-hero-action-row">
            ...
            <button className="btn-outline report-hero-share" onClick={() => setSecureDialogMode('share')}>
              <Share2 size={14} /> Secure Share
            </button>
          </div>
          <h2 className="report-title">{uploadedData.datasetType} Analysis Report</h2>
          <ul className="report-overview-bullets">
            {reportOverviewBullets.map((bullet, index) => <li key={index}>{bullet}</li>)}
          </ul>
        </div>
