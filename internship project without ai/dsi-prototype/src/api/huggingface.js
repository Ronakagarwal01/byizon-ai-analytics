...
export async function askDataChat(question, data, history = [], conversationId = null) {
  ...
  if (!data) {
    return askBackendChat(question, null, history, conversationId);
  }

  if (connectedActionRequest) {
    return askBackendChat(question, data, history, conversationId);
  }

  const local = () => localAnswer(question, data);
  if (data.engineType === 'python-pandas') {
    try {
      return await askBackendChat(question, data, history, conversationId);
    } catch (error) {
