...
import { createJsonDashboard, createProtectedShare, downloadProtectedPdf, revokeProtectedShare } from '../api/universalBackend';
...
export default function SecureExportDialog({ open, mode, data, customization = null, onClose }) {
  ...
  const [resultCanRevoke, setResultCanRevoke] = useState(false);
    ...
    try {
      if (mode === 'share') {
        if (customization) {
          ...
          setResultLink(`${window.location.origin}/custom-dashboard/${share.shareId}`);
          setResultCanRevoke(true);
        } else {
          const dashboard = await createJsonDashboard(data.sessionId, data, {
            prompt: 'Create protected JSON dashboard from current uploaded analysis',
          });
          setResultShareId(dashboard.dashboardId);
          setResultPassword(dashboard.password);
          setResultLink(`${window.location.origin}/dashboard/${dashboard.dashboardId}`);
          setResultCanRevoke(false);
        }
      } else {
      ...
      <section className="secure-dialog" role="dialog" aria-modal="true" aria-labelledby="secure-dialog-title">
        ...
        {resultLink ? (
          <div className="secure-share-result">
            ...
            <div className="secure-share-link-row">
              <input className="secure-share-link-input" value={resultLink} readOnly aria-label="Protected share link" />
            ...
            {resultCanRevoke ? (
              ...
              <span className="secure-dashboard-note">This link renders from stored dashboard JSON at /dashboard/{resultShareId}.</span>
            )}
          ...
          <form onSubmit={submit}>
            <label className="share-expiry-field">
              ...
              <select className="secure-expiry-select" value={expiresInDays} onChange={event => setExpiresInDays(Number(event.target.value))}>
