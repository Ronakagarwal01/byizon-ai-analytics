...
const VISUAL_COLORS = ['#2563eb', '#4f46e5', '#7c5cff', '#06b6d4', '#0d9488', '#10b981'];
const EMPTY_ARRAY = [];
const EMPTY_OBJECT = {};
        ...
        <BarChart data={data.slice(0, 12)} margin={{ top: 8, right: 12, left: 0, bottom: 42 }}>
          ...
          <Bar dataKey="numericValue" name="Value" fill={`url(#${gradientId})`} radius={[10, 10, 3, 3]}>
            {data.slice(0, 12).map((entry, i) => (
              <Cell key={`${entry.name}-${i}`} fill={VISUAL_COLORS[(index + i) % VISUAL_COLORS.length]} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        <div className="page-header" style={{ marginBottom: 20 }}>
          <div>
            <span className="premium-page-eyebrow"><Sparkles size={14} /> AI Dashboard Generator</span>
            <h1 className="page-title">Welcome back, {firstName}! 👋</h1>
            <p className="page-subtitle">{uploadedData.fileName} dashboard is ready with live KPIs, charts, filters, and secure sharing.</p>
          </div>
