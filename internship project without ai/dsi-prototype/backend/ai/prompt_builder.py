...
def build_prompt(
    *,
    ...
    safe_history = _compact(conversation_history or [])[-25:] if conversation_history else []
    is_conversational = (business_context or {}).get("purpose") == "chat_answer"
    prompt_sections = {
        ...
        "outputStyle": (
            {
                "format": "Natural conversational answer; do not force headings or a fixed template.",
                "language": "Match the user's Hindi, English, Hinglish, or Roman Hindi.",
                "followUps": "Use recent conversation to understand references and follow-up questions.",
            }
            if is_conversational
            else {
                "sections": [
                    "Summary", "Key Insights", "Important KPIs", "Observed Trends",
                    "Possible Reasons", "Business Recommendations", "Confidence",
                ]
            }
        ),
