...
async def app(scope, receive, send):
    ...
    try:
        ...
        if method == "GET" and path.startswith("/api/oauth/start/"):
            ...
            await _send_redirect(send, oauth_start(connector_id, query.get("returnUrl"), workspace_user_id))
            return
        ...
        if method == "POST" and path == "/api/connections/import":
            ...
            safe_name = _sanitize_filename(file_name)
            result = analyze_file(safe_name, content)
            _attach_source_provenance(result, content, "connected_tool", safe_name)
            ...
            session = create_session(result, {"fileName": safe_name, "source": "connected_tool", "connectionId": connection_id, "resourceId": resource_id}, workspace_user_id)
        ...
        if method == "POST" and path == "/api/connections/refresh-source":
            ...
            safe_name = _sanitize_filename(file_name)
            result = analyze_file(safe_name, content)
            _attach_source_provenance(result, content, "connected_tool_refresh", safe_name)
            result["connectedSource"] = connected_source_metadata(connection_id, resource)
            clear_session(payload.get("sessionId"), workspace_user_id)
            session = create_session(result, {"fileName": safe_name, "source": "connected_tool_refresh", "connectionId": connection_id, "resourceId": resource["id"]}, workspace_user_id)
            mark_connection_synced(connection_id, workspace_user_id)
        ...
        if method == "POST" and path == "/api/analyze":
            ...
            result = analyze_file(safe_name, upload["content"])
            _attach_source_provenance(result, upload["content"], "manual_upload", safe_name)
            session = create_session(result, owner_user_id=workspace_user_id)
            await _send_json(send, 200, {"ok": True, "sessionId": session["sessionId"], "analysis": session["analysis"]})
            return
        ...
        if method == "POST" and path == "/api/chat":
            ...
            session = get_session(session_id, workspace_user_id)
            analysis = session["analysis"] if session else payload.get("analysis")
            ...
            answer = chat_answer(question, analysis)
            append_chat(session_id, "assistant", answer, workspace_user_id) if session_id else None
            await _send_json(send, 200, {"ok": True, "answer": answer, "sessionId": session_id})
            return
