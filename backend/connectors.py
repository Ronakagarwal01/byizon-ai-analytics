...
def _send_gmail_command(
    ...
    elif re.search(r"\b(link|url|share|send|bhejo|email|mail)\b", normalized):
        urls = re.findall(r"https?://[^\s\"'<>]+", question)
        body = f"Shared link:\n{urls[-1].rstrip('.,)')}" if urls else ""
    else:
        body = ""
    if not body:
        return {
    ...
    if not subject:
        ...
        elif body.startswith("Shared link:"):
            subject = "Shared link"
        else:
...
def _execute_google_command(
    ...
    if not any((gmail_intent, calendar_intent, meet_intent, doc_intent, sheet_intent)):
        ...
        return None
    capability = "gmail" if gmail_intent else "meet" if meet_intent else "calendar" if calendar_intent else "docs" if doc_intent else "sheets"
...
def execute_connected_command(
    ...
    if send_intent and slack_intent:
        current_message = _current_user_message(question)
        normalized_current = _match_text(current_message)
        connections = [
            item for item in _owned_connections(owner_user_id)
        ...
        requested_match = re.search(r"#\s*([\w-]+(?:\s+[\w-]+)*)", current_message)
        requested = _match_text(requested_match.group(1)).replace(" ", "-") if requested_match else ""
        if not requested and "conversation memory" in normalized:
            requested = normalized_current.replace(" ", "-")
        matching = [
            channel for channel in channels
            if any(
                (requested and _match_text(str(name)).replace(" ", "-") == requested)
                or (not requested and _match_text(str(name)) in normalized_current)
                or (not requested and _match_text(str(name)).replace("-", " ") in normalized_current)
...
def resolve_connected_resource(question: str, owner_user_id: str | None = None) -> dict[str, Any] | None:
    ...
    current_message = _current_user_message(question)
    normalized_current = _match_text(current_message)
    selected_option = normalized_current.replace(" ", "-") if "conversation memory" in normalized_question else ""
    import_intent = any(
    ...
    matching_channels = [
        ...
        and any(
            (_match_text(str(name)).replace(" ", "-") == selected_option)
            if selected_option
            else (_match_text(str(name)) in normalized_question)
            for name in resource.get("channelNames", [])
        )
    ]
...
def _current_user_message(question: str) -> str:
    match = re.search(r"(?:^|\n)Current user message:\s*(.+)", question, flags=re.IGNORECASE)
